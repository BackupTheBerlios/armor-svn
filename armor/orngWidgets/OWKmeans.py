...
class OWKmeans(OWWidget):
    ...
    def setData(self,data):
        if not data:
            pass
        self.data = data
        self.kmeans = armor.kmeans.kmeansObj(data, numClusters = self.numClusters, maxiter = self.maxiter, numruns = self.numruns)
        self.send("Codebook", self.kmeans.outContainer)
