...
class OWSift(OWWidget):
	...
	print data
        self.data = data
        self.sift = sift.sift(images=data, Octave=self.Octave, Levels=self.Levels, FirstOctave=self.FirstOctave, PeakThresh=self.PeakThresh, EdgeThresh=self.EdgeThresh, Orientations=self.Orientations)
	# Copy the generator so different widgets get different generators
        if self.useGenerator:
	    output = self.sift.iterator
        else:
            self.sift.createDescr()
	    output = self.sift.descriptors
