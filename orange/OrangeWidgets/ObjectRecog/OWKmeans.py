...
class OWKmeans(OWWidget):
    ...
    def setData(self,data):
        if not data:
            pass
        self.data = data
        self.kmeans = kmeans.kmeans(data, numClusters = self.numClusters, maxiter = self.maxiter, numruns = self.numruns)
...
#        debug_here()
	...
	# Create exampleTable
	histoList = []
	for d in self.kmeans.dataHistogram:
	    histoList.append(list(d[0]) + [d[1]])
	    
        domain = orange.Domain([orange.IntVariable('a%i'%x) for x in xrange(len(self.kmeans.dataHistogram[0][0]))] + [orange.StringVariable("class")])
