...
void randperm(unsigned int *order, unsigned int npoints)
{
	double *r;
	unsigned int i;
	r = (double*)malloc(2*npoints*sizeof(double));
...
double compute_sserror(const double *CX, const PREC *X, const unsigned int *c,unsigned int dim, unsigned int npts)
{
	unsigned int i;
	double sse, d;
	const double *pcx;
	...
	{
		pcx = CX+c[i]*dim;
		d = (double)(compute_distance(px,pcx,dim));
		sse += d*d;
	}
	return(sse);
}

void compute_cluster_distances(BOUND_PREC *dist, BOUND_PREC *s, const double *CX, unsigned int dim,unsigned int nclus, const bool *cluster_changed)
{
	unsigned int j,i,cnt;
	const double *pcxp, *pcx;
...
void remove_point_from_cluster(unsigned int cluster_ind, double *CX, const PREC *px, unsigned int *nr_points, unsigned int dim)
{
	double *pcx;
	...
	else
	{
		...
		for ( k=0 ; k<dim ; k++ )
			pcx[k] = (nr_old*pcx[k] - (double)px[k])/nr_new;
	}
}

void add_point_to_cluster(unsigned int cluster_ind, double *CX, const PREC *px, unsigned int *nr_points, unsigned int dim)
{
	unsigned int k;
	double nr_old, nr_new;
	double *pcx;
...
unsigned int init_point_to_cluster(unsigned int point_ind, const PREC *px, const double *CX, unsigned int dim,unsigned int nclus, double *mindist, BOUND_PREC *low_b, const BOUND_PREC *cl_dist)
{
	double mind, d;
...
unsigned int assign_point_to_cluster(unsigned int point_ind, const PREC *px, const double *CX, unsigned int dim,unsigned int nclus, unsigned int old_assignment, double *mindist, BOUND_PREC *s, BOUND_PREC *cl_dist, BOUND_PREC *low_b)
{
	double mind, d;
...
double kmeans_run(double *CX,const PREC *X,unsigned int *c,unsigned int dim,unsigned int npts,unsigned int nclus,unsigned int maxiter)
{
	double *tCX;
	unsigned int *CN, *old_c, *rperm = NULL;
	double sse = 0.0;
	double *pcx, *tpcx, *mindist;
	...
	cl_dist = (BOUND_PREC *)calloc(nclus*nclus, sizeof(double));
	assert(cl_dist);
	...
	mindist = (double *)malloc(npts * sizeof(double));
	assert(mindist);
	for (i=0;i<npts;i++)
		mindist[i] = DBL_MAX;

	cluster_changed = (bool *) malloc(nclus * sizeof(bool)); /* did the cluster changed? */
	...
	while (iteration < maxiter || maxiter == 0)
	{
		...
		for ( j=0,pcx=CX,tpcx=tCX; j<nclus ; j++,pcx+=dim,tpcx+=dim )
		{
			...
			if (cluster_changed[j])
			{
				...
				memcpy(pcx,tpcx,dim*sizeof(double));
			}
		}
...
double kmeans(double *CX,const PREC *X,unsigned int *assignment,unsigned int dim,unsigned int npts,unsigned int nclus,unsigned int maxiter, unsigned int restarts)
{
	...
	if (res>0)
	{
		minsse = sse;
		order = (unsigned int*)malloc(npts*sizeof(unsigned int));
		bestCX = (double*) malloc(dim*nclus*sizeof(double));
		bestassignment = (unsigned int*)malloc(npts*sizeof(unsigned int));

		memcpy(bestCX,CX,dim*nclus*sizeof(double));
		...
		memcpy(CX,bestCX,dim*nclus*sizeof(double));
